...
class VisitSchedulerClient(
  ...
  fun getAvailableVisitSessions(prisonId: String, prisonerId: String, sessionType: SessionType, min: Int?, max: Int?): List<AvailableVisitSessionDto>? {
    return webClient.get()
      .uri("/visit-sessions/available") {
        visitAvailableSessionsUriBuilder(prisonId, prisonerId, sessionType, min, max, it).build()
      }
      .accept(MediaType.APPLICATION_JSON)
      .retrieve()
  ...
  private fun visitAvailableSessionsUriBuilder(prisonId: String, prisonerId: String, sessionType: SessionType, min: Int?, max: Int?, uriBuilder: UriBuilder): UriBuilder {
    uriBuilder.queryParam("prisonId", prisonId)
    uriBuilder.queryParam("prisonerId", prisonerId)
    uriBuilder.queryParam("sessionType", sessionType.toString())
    uriBuilder.queryParamIfPresent("min", Optional.ofNullable(min))
    uriBuilder.queryParamIfPresent("max", Optional.ofNullable(max))
    return uriBuilder
  }
