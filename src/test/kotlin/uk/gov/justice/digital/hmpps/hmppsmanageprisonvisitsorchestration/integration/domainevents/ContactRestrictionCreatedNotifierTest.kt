...
import uk.gov.justice.digital.hmpps.hmppsmanageprisonvisitsorchestration.service.listeners.notifiers.CONTACT_RESTRICTION_CREATED_TYPE
...
class ContactRestrictionCreatedNotifierTest : PrisonVisitsEventsIntegrationTestBase() {

  @Test
  fun `when valid contact global restriction created event received then event is successfully processed`() {
    // Given
    ...
    val domainEvent = createDomainEventJson(
      CONTACT_RESTRICTION_CREATED_TYPE,
    ...
    val publishRequest = createDomainEventPublishRequest(CONTACT_RESTRICTION_CREATED_TYPE, domainEvent)
    ...
    // Then
    assertStandardCalls(contactRestrictionCreatedNotifierSpy, VISIT_NOTIFICATION_CONTACT_RESTRICTION_UPSERTED_PATH, sentRequestToVsip)
  }

  @Test
  fun `when invalid global contact restriction created event received then event is not processed`() {
    // Given
