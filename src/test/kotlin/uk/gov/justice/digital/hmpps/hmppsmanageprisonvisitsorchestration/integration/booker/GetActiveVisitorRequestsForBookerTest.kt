...
import uk.gov.justice.digital.hmpps.hmppsmanageprisonvisitsorchestration.controller.GET_VISITOR_REQUESTS_BY_BOOKER_REFERENCE_AND_PRISONER_ID
...
class GetActiveVisitorRequestsForBookerTest : IntegrationTestBase() {
  ...
  fun `when get active visitor requests is called without correct role then UNAUTHORIZED status is returned`() {
    // When
    val url = GET_VISITOR_REQUESTS_BY_BOOKER_REFERENCE_AND_PRISONER_ID.replace("{bookerReference}", "bookerReference")
